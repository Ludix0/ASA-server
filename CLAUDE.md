...
- **`ServerAdminPassword` passé en option `-` (`construire_options_lancement`), jamais dans la chaîne `?clé=valeur` (`construire_parametres_partie`)** : bug connu d'ARK — placé dans cette dernière, son parseur avale tout ce qui suit dans la chaîne au lieu de s'arrêter au `?` suivant. Constaté concrètement dans le vrai `GameUserSettings.ini` généré (`ServerAdminPassword` contenait `Port=`, `RCONPort=`, `SessionName=`... jusqu'à `ImplantSuicideCD=28800`, tous collés dans sa propre valeur). `ServerPassword`, lui, n'a pas ce problème et reste dans la chaîne `?clé=valeur`.

## Cryopods hors base et événements saisonniers automatiques

- **Cryopods utilisables loin de la base** : trois réglages dans `docker-compose.yml`/`start_server.sh` — `DisableCryopodFridgeRequirement=True` (déploiement/rappel sans Cryofridge à proximité), `AllowCryoFridgeOnSaddle=True` (Cryofridge constructible sur une selle de plateforme — reste utile car un Cryofridge est toujours nécessaire pour **fabriquer/recharger** les cryopods, ce que `DisableCryopodFridgeRequirement` ne change pas), `DisableCryopodEnemyCheck=True` (ignore la présence ennemie à proximité — surtout utile en PvP, notre serveur étant en `ServerPVE=True`). **Important** : ARK ne lit pas ces trois clés depuis les paramètres de lancement (`?clé=valeur`, contrairement à la plupart des autres réglages de ce projet) — elles doivent être écrites directement dans `GameUserSettings.ini`, section `[ServerSettings]`. `appliquer_reglages_cryopod()` dans `start_server.sh` s'en charge juste avant le lancement du jeu, via la fonction générique idempotente `appliquer_reglage_ini()` (crée le fichier/la section si absents, met à jour la valeur sans dupliquer la ligne à chaque redémarrage du conteneur).
...
- **`GameUserSettings.ini` (paramètres de lancement + cryopods) n'est pas le seul fichier de config d'ARK** : `Game.ini` (section `[/script/shootergame.shootergamemode]`) couvre un ensemble de réglages distinct (friendly fire, XP détaillée par activité, bascules apprivoisement/monte/reproduction, etc.), ni lisible ni modifiable via les paramètres `?clé=valeur`/`-options` du lancement — jusqu'ici totalement absent de ce projet.
- `appliquer_reglages_game_ini()` dans `start_server.sh` (appelée juste après `appliquer_reglages_cryopod`, avant le lancement du jeu) écrit dans `Game.ini` via la même fonction générique `appliquer_reglage_ini()` que pour les cryopods. Réglages actuellement couverts (variables dans `docker-compose.yml`, toutes câblées à la valeur par défaut d'ARK — aucun changement de comportement tant que Ludo ne les modifie pas) :
  - `bDisableFriendlyFire`, `bPvEDisableFriendlyFire`, `bAutoPvETimer`, `PreventOfflinePvPConnectionInvincibleInterval` — ces 4 réglages PvP n'ont aucun effet réel tant que `ServerPVE=True` (notre serveur est déjà intégralement en PvE), gardés pour le jour où ça changerait.
  - `GenericXPMultiplier`, `KillXPMultiplier`, `CraftXPMultiplier`, `HarvestXPMultiplier` — multiplicateurs d'XP par activité, cumulatifs avec `XPMultiplier` (paramètre de lancement classique, déjà existant).
  - `bDisableDinoTaming`, `bDisableDinoRiding`, `bDisableDinoBreeding`.
  - D'autres catégories existent côté ARK (multiplicateurs de dégâts/récolte par espèce, limites de tourelles, récompenses Genesis/Hexagone...) mais n'ont pas été demandées — à ajouter au besoin sur le même modèle si Ludo les veut un jour.
- **`appliquer_reglage_ini()` corrigée pour accepter des noms de section contenant des `/`** : son adresse `sed` pour insérer une nouvelle clé sous une section utilisait `/` comme délimiteur (`sed -i "/^\[${section}\]/a ..."`), ce qui cassait dès qu'un nom de section contenait lui-même un `/` — cas de `/script/shootergame.shootergamemode` (Game.ini). Délimiteur changé pour `|` (syntaxe `sed -i "\|^\[${section}\]|a ..."`), compatible avec les deux fichiers ini du projet (`ServerSettings` pour les cryopods, `/script/shootergame.shootergamemode` pour Game.ini).
